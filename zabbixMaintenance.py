...
def main():
  ...
  if None == args.hostname or None == args.hostgroup:
  ...
  if args.delete :
    ...
    zm = ZabbixMaintenance( args.url, args.user, args.password )
    if args.hostname:
      zm.endMaintenanceMode( args.hostname,args.maintenance_length )
    elif args.hostgroup:
      zm.endMaintenanceModeHostgroup( args.hostgroup,args.maintenance_length )
    else:
      print "Error creating maintenance mode, no hostname or group specified"
  else:
...
class ZabbixMaintenance:
    ...
    def deleteHostGroup(self,hostgroup):
      _hosts = self.getHostGroupByName(hostgroup)
      hosts = []
      for h in _hosts:
