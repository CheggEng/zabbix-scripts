...
parser.add_argument('--maintenance-length', help='How long in seconds to be in maintenance mode. (default: 3600)', default=3600)
...
class ZabbixMaintenance:
    ...
    def maintenanceMode(self,args,timeWindow):
      ...
      request_args = {
        "name": "Generated by {0} on {1} for {2}".format(basename(__file__),currentDate,args.hostname),
        "active_since": int(time.time()),
        "active_till": int(time.time()+int(timeWindow)),
        "hostids": [
          _host['hostid']
        ],
        "timeperiods": [
